...
from human_mouse_movement import HumanMouseMovement
# ---------------------------------------
# Khởi tạo các lock
# ---------------------------------------
...
try:
    with successupload_lock:
        with open("successupload.txt", "r") as f:
        ...
        if item_type == "video":
            try:
                with profile_video_lock: # Đảm bảo an toàn khi đọc profile_video.txt
                    with open("profile_video.txt", "r", encoding="utf-8") as f:
                        for line in f:
                            line = line.strip()
            ...
            if item_type == "video":
                ...
                try:
                    with profile_video_lock: # Đảm bảo an toàn khi ghi vào profile_video.txt
                        with open("profile_video.txt", "a", encoding="utf-8") as f:
...
def process_profile(thread_id, proxy_data, window_pos):
    ...
    while True:
        ...
        try:
            ...
            human_mouse = HumanMouseMovement(driver, window_size=(1800, 1080), window_scale=0.35)
        except Exception as e:
        ...
        video_path = get_random_item(videos, "use_video.txt", profile_id, item_type="video")
        if not video_path:
        ...
        # 7) Nhập hashtag
        # ---------------------------
        hashtags = load_list("hashtag.txt")
        if not hashtags:
        ...
        try:
            ...
            human_mouse.click_element(post_button)
        ...
        try:
            ...
            with music_lock:  # Thêm lock ở đây
                music_list = load_list("clidtiktok.txt")
            if not music_list:
                print(f"Thread {thread_id}: Không có nhạc trong file clidtiktok.txt")
                close_profile(driver, profile_id)
                continue

            music_name = get_random_item(music_list, "use_music.txt", profile_id, item_type="music")
            ...
            human_mouse.click_element(search_input)
            ...
            human_mouse.click_element(final_post_button)
        ...
        try:
            ...
            if len(appeal_buttons) >= 2:
                human_mouse.click_element(appeal_buttons[0])
                time.sleep(1)
                human_mouse.click_element(appeal_buttons[1])
        ...
        try:
            with successupload_lock:
                with open("successupload.txt", "a") as f:
                    f.write(str(profile_id) + "\n")
        except Exception as e:
